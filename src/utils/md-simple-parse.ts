...
const antiList = (list: string[]) => list.map((e) => e.replace(/^-/, '').trim())
...
const splitFistLine = (s: string) => [s.slice(0, s.indexOf('\n')), s.slice(s.indexOf('\n') + 1)]
...
const parseSkills = (s: string) => {
  ...
  const title = antiHH(l1)
  const skills = list.map((e) => e.split(/[:：]\s?/)).map(([label, list]) => ({ label: label.replace('- ', ''), list }))
...
const parseTraits = (s: string) => {
  const [l1, ...list] = s.split('\n')
  const title = antiHH(l1)
  const traits = list.map((e) => e.split(/[:：]\s?/)).map(([label, list]) => ({ label: label.replace('- ', ''), list }))
...
export const parseMd = (file: string) => {
  ...
  const [blockHeader, blockEdu, blockSkills, blockTraits, blockWorks, blockProjs] = blocks

  return {
    header: parseHeader(blockHeader),
    edu: parseEdu(blockEdu),
    skills: parseSkills(blockSkills),
    traits: parseTraits(blockTraits),
